...
app.get('/get-user', authenticateToken, async (req, res) => {
    const { user } = req; // Fixed to extract `user` from `req` directly
    if (!user) {
        return res.sendStatus(401);
    }
    const isUser = await User.findOne({ _id: user._id }); // Ensure this line runs only if `user` exists
    if (!isUser) {
        return res.sendStatus(404); // Optional, in case the user is not found in the database
    }
    return res.json({
        user: {
